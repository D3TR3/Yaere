...
import { sendMessage, markMessagesAsRead, addReaction, removeReaction } from '../firebase/firestore';
...
const ChatBox = () => {
    ...
    const shouldUpdate = useCallback((key) => {
        ...
        return false;
    }, [CACHE_EXPIRATION]); // Add CACHE_EXPIRATION as dependency
                ...
                activeListenersRef.current[listenerKey] = onSnapshot(
                    ...
                    async (messageSnap) => {
                        if (!messageSnap.empty) {
                            ...
                            if (hasUnread && (!selectedFriend || selectedFriend.id !== friendId)) {
        ...
        const processFriendsUpdate = debounce(async (snapshot) => {
            ...
            try {
                const friendsPromises = snapshot.docs.map(async (docSnapshot) => {
                    ...
                    const hasUnread = lastMessage && !lastMessage.read && lastMessage.senderId !== currentUser.uid;
                ...
                const sortedFriends = friendsList.sort((a, b) => b.lastActivity - a.lastActivity);
    ...
    }, [currentUser, shouldUpdate, selectedFriend, friends, notificationPermission]);
    ...
    const handleScroll = useCallback(async () => {
        ...
        if (!container || isLoadingMore || !hasMore || !lastMessageRef.current) return;
        ...
        if (container.scrollTop === 0) {
            ...
            try {
                ...
                const oldMessages = snap.docs.map(doc => ({
                    id: doc.id,
                    ...doc.data(),
                    timestamp: doc.data().timestamp?.toDate()
                })).reverse();
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            const chatId = [currentUser.uid, selectedFriend.id].sort().join('_');
            await sendMessage(currentUser.uid, selectedFriend.id, message.trim(), replyingTo);
            setMessage('');
            setReplyingTo(null);
    ...
    const MessageBubble = ({ message, isLastMessage }) => {
        ...
        const repliedMessage = message.replyTo ? messages.find(m => m.id === message.replyTo.id) : null;
            ...
            <div className={`flex items-end gap-2 ${isCurrentUser ? 'justify-end' : 'justify-start'} 
                ...
                <div className="relative min-w-0 max-w-[70%] md:max-w-[85%] group">
                    ...
                    <div className={`absolute top-1/2 -translate-y-1/2 flex gap-1
                                 ...
                                 ${isCurrentUser ? 'right-full -translate-x-1' : 'left-full translate-x-1'}`}>
                        {/* Reply button */}
                        <button
                            onClick={(e) => {
                                e.preventDefault();
                        ...
                        <button
                            onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                                setActiveReactionMessage(activeReactionMessage?.id === message.id ? null : message);
                            }}
                            onTouchStart={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
                    ...
                    {/* Message content */}
                    <div className={`relative px-3 md:px-4 py-2 md:py-2.5 rounded-2xl transform transition-all duration-200
                        ${isCurrentUser
                        ...
                        {repliedMessage && (
                            <div className={`text-sm mb-2 pb-2 border-b ${isCurrentUser ? 'border-gray-300' : 'border-gray-600'}
                                        overflow-hidden`}>
                                <p className={`font-medium ${isCurrentUser ? 'text-gray-600' : 'text-gray-400'}
                                          text-ellipsis overflow-hidden`}>
                                    {repliedMessage.senderId === currentUser.uid ? 'You' : selectedFriend.displayName}
                                </p>
                                <p className="truncate opacity-75 max-w-full">{repliedMessage.text}</p>
                            </div>
                        )}
                        <div className="space-y-1">
                            ...
                            <p className={`text-[11px] text-right ${isCurrentUser ? 'text-gray-600' : 'text-gray-400'}`}>
                            ...
                            <div className={`absolute -bottom-2 flex items-center gap-0.5 
                                ...
                                {Object.entries(reactionCounts).map(([reaction, count]) => (
                                    <div key={reaction} 
                                        ...
                                        <span className="text-base">{getReactionEmoji(reaction)}</span>
                                        {count > 1 && (
                        ...
                        {showReactionPanel && (
                            <>
                                ...
                                <div className={`absolute w-auto
                                    ...
                                    <div className="reaction-panel bg-[#1A1A1A] rounded-lg 
                                        ...
                                        {[
                                            ...
                                            <button
                                                ...
                                                className={`p-2 md:p-1.5 hover:bg-[#272727]/80 rounded-lg
                                                          ...
                                                          ${message.reactions?.[currentUser.uid] === reaction.name 
                                                            ? 'bg-[#272727] scale-110' 
                                                            : 'hover:bg-opacity-50'}`}
                                            >
        ...
        return (
            <div className="px-4 pb-2 flex justify-end">
                <div className="inline-flex items-center">
                    <p className={`text-xs ${lastMessage.read ? 'text-white' : 'text-gray-400'}`}>
            ...
            >
                <div className="flex-1 min-w-0 overflow-hidden">
                    <p className="text-white/90 text-sm font-medium truncate">
                        Replying to {replyingTo.senderId === currentUser.uid ? 'yourself' : selectedFriend.displayName}
                    </p>
                    <p className="text-gray-500 text-sm truncate max-w-full">{replyingTo.text}</p>
                </div>
                <button
            ...
            // Check if data is an object with action and users (from remove/add operations)
                ...
                if (data.action === 'remove') {
                    setFriends(prev => prev.filter(friend => 
                        !data.users.some(user => user.id === friend.id)
                    ));
                    // If the removed friend was selected, clear selection
                    if (selectedFriend && data.users.some(user => user.id === selectedFriend.id)) {
                        setSelectedFriend(null);
                    }
                } else if (data.action === 'add') {
                    // Add new friend to the list if they're not already there
                    const newFriend = data.users.find(user => user.id !== currentUser.uid);
                    if (newFriend) {
                        setFriends(prev => {
    ...
    useEffect(() => {
        let unsubscribe;
        if (currentUser) {
            unsubscribe = subscribeToFriendsList(currentUser.uid, (friendsList, changes) => {
                // Update friends list
    ...
    return (
        <div className="fixed inset-x-0 bottom-0 top-24 md:static md:inset-auto flex flex-col md:flex-row md:h-[750px] 
            ...
            <div className={`w-full md:w-[350px] h-full flex flex-col border-b md:border-b-0 md:border-r 
                        ...
                        {friends.map((friend) => (
                            <div
                                ...
                                className={`w-full flex items-center gap-3 p-3 rounded-lg transition-all duration-200
                                    ...
                                    ${selectedFriend?.id === friend.id ? 'bg-[#272727]' : ''}`}
                            >
                                <img
                                ...
                                <div className="text-left flex-1 min-w-0">
                                    <p className="text-white font-medium transition-all duration-200 
                        ...
                        <div 
                            ...
                            style={{
                                transform: touchStart && touchEnd ? 
                                    `translateX(${Math.min(Math.max(touchEnd - touchStart, 0), 200)}px)` : 
                                    'translateX(0)',
                        ...
                        >
                            ...
                            <div className="p-4 border-b border-[#272727] backdrop-blur bg-[#020202]/80 sticky top-0 z-10 flex items-center gap-4">
                                ...
                                <div className="flex items-center gap-3 group">
                                    ...
                                    <div>
                                        <h3 className="text-white font-medium">{selectedFriend.displayName}</h3>
                                    </div>
                                </div>
                            </div>
                            ...
                            >
                                {isLoadingMore && (
                                    <div className="text-center text-gray-400 py-2">Loading more messages...</div>
                                )}
                                {messages.length === 0 ? (
                                    <div className="flex flex-col items-center justify-center h-full space-y-4 text-gray-400">
                                        ...
                                        />
                                        <p className="text-lg">No messages yet with {selectedFriend.displayName}</p>
                                        <p className="text-sm flex items-center gap-2">
                                            Say hello! <RiWechatLine className="text-lg animate-bounce" />
                                        </p>
                                    </div>
                                ) : (
                                    <>
                                    ...
                                    <form onSubmit={handleSendMessage} className="flex gap-2 max-w-full">
                                        ...
                                        <button
                                            ...
                                            className={`p-2.5 text-gray-400 hover:bg-[#272727] rounded-xl transition-all duration-200
                                                ...
                                                ${isMessageCooldown ? 'cursor-not-allowed opacity-50' : ''}`}
                                            title={isMessageCooldown ? 'Please wait before sending another message' : ''}
                                        >
